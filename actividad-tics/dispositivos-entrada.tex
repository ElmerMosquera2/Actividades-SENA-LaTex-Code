...
\begin{titlepage}
    ...
    \textbf{DESARROLLO DE TRABAJO ESCRITO SOBRE DISPOSITIVOS DE ENTRADA} \par
    \vspace{2.5cm}

    \textbf{INSTRUCTOR:} \\
    \textbf{NOMBRE DEL INSTRUCTOR} \par
...
\subsection{¿Qué es un dispositivo de entrada?}
...
\subsection{¿Qué es un micrófono?}
...
\subsection{El micrófono como interfaz humano-computador}
...
\subsection{Parámetros técnicos importantes}
...
\subsection{Futuro de los micrófonos}
...
\subsection{Historia de la cámara web}
...
\subsection{¿Qué son las cámaras web?}
...
\subsection{Tipos de cámaras web}

\textbf{Imágenes de referencia:}

\begin{figure}[h]
    \centering
    \caption{Tipos de WebCam}
...
\subsection{Resoluciones de las cámaras web}
...
\bottomrule
\end{tabular}
\caption{Resoluciones comunes de cámaras web}
\label{tab:resoluciones}
\end{table}

\subsection{¿Por qué son importantes los FPS?}
...
\bottomrule
\end{tabular}
\caption{Relación de FPS y fluidez de video}
\label{tab:fps}
\end{table}
...
\subsection{Uso de las cámaras web en la vida cotidiana}
...
\begin{figure}[h]
    \centering
    \caption{Streaming}
    % ¡IMPORTANTE! Esta imagen está duplicada (usa el mismo archivo que Tipos_de_webcam.jpg).
    % Reemplazar por la imagen correcta de streaming.
    \includegraphics[width=0.4\linewidth]{assets/images/Tipos_de_webcam.jpg}
    \label{fig:Streaming}
\end{figure}
